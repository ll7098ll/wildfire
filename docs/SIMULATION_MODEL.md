...
시뮬레이터는 가로·세로 $1,000\text{m} \times 1,000\text{m}$의 실제 산악 지형 영역을 $128 \times 128$개의 정사각형 셀(Cell)로 분할합니다.

* **그리드 크기 ($N$):** $128 \times 128 = 16,384\text{ cells}$
* **셀 간격 ($\Delta s$):**
  $$\Delta s = \frac{1000\text{m}}{128 - 1} \approx 7.874\text{m}$$
* **단일 셀 면적 ($A_{cell}$):**
  $$A_{cell} = \Delta s^2 \approx 62.0\text{m}^2 \approx 0.0062\text{ha}$$

각 발화 셀은 **무어 이웃(Moore Neighborhood, 8방향)**에 위치한 미연소 이웃 셀로 화염 전파를 시도합니다:
* **직교 이웃 (상, 하, 좌, 우 4개):** 거리 $d = \Delta s$
* **대각 이웃 (대각선 4개):** 거리 $d = \sqrt{2} \cdot \Delta s \approx 11.135\text{m}$
...
식생의 밀도($\text{Fuel} \in [0, 1]$), 상대 습도($H \in [10, 90]\%$), 대기 온도($T \in [15, 42]^\circ\text{C}$)로부터 기본 발화 가능성을 산출합니다:

$$R_{base} = 0.15 \times \text{Fuel} \times \text{Dryness}(H) \times \text{TempFactor}(T)$$

* **건조도 계수 $\text{Dryness}(H)$:**
  $$\text{Dryness}(H) = \max\left(0.20, \; \frac{100 - 0.85 \cdot H}{100}\right)$$
  *(습도 10%일 때 약 0.915로 매우 건조하여 발화 촉진, 습도 80%일 때 약 0.32로 연소 억제)*
* **온도 계수 $\text{TempFactor}(T)$:**
  $$\text{TempFactor}(T) = \min\left(1.60, \; \max\left(0.80, \; \frac{T}{26}\right)\right)$$
  *(기준 온도 26℃ 기준, 40℃ 이상의 혹서기에는 1.5배 이상 발화율 증가)*
...
1. **상향 확산 ($\tan \theta > 0$, 오르막):**
   $$\Phi_{slope} = 1.0 + \min\left(4.20, \; 3.20 \cdot (\tan \theta)^{1.10}\right)$$
   *(급경사 능선에서는 평지 대비 최대 5.2배까지 전파 속도가 가속됨)*

2. **하향 확산 ($\tan \theta \le 0$, 내리막):**
   $$\Phi_{slope} = \max\left(0.40, \; \frac{1.0}{1.0 + 1.80 \cdot |\tan \theta|}\right)$$
   *(내리막길에서는 대류열이 지표면 반대편으로 방출되어 전파 속도가 최대 60% 감속됨)*

---

### 3.3 풍향·풍속 벡터 효과 ($\Phi_{wind}$)

풍향 $\phi$ (방위각 도, 0°는 북쪽을 향해 부는 바람)와 풍속 $W_{speed}$ (m/s)로부터 2D 풍속 단위 벡터 $\vec{W}$를 정의합니다:
...
1. **순풍 ($\cos \alpha > 0$, 바람 부는 방향으로 확산):**
   ...
   *(강풍 $20\text{m/s}$ 하에서 순풍 방향 화선은 평시 대비 15~20배까지 맹렬히 추진)*

2. **역풍 ($\cos \alpha \le 0$, 바람을 거슬러 확산):**
   $$\Phi_{wind} = \max\left(0.22, \; \frac{1.0}{1.0 + \left(\frac{W_{speed}}{7.0}\right) \cdot |\cos \alpha|}\right)$$
   *(역풍이어도 복사열로 인해 미약한 역방향 확산은 지속되며 하한값 0.22 유지)*
...
* 이 방식을 통해 프레임 레이트($\Delta t$)가 달라져도 일정한 물리적 확산 속도를 유지합니다.

---

## 5. 비화 (Spotting Fire, 불씨 도약) 모델

강풍($W_{speed} > 8\text{m/s}$)과 활성 화선이 결합하면 수관화(Crown Fire)에서 타오르는 불씨가 대기 중으로 솟구쳐 풍하측 원거리로 날아가는 **비화 현상**이 발생합니다.

* **비화 발생 확률 (초당):**
  $$P_{spot} = 0.008 \times \Delta t \times \left(\frac{W_{speed}}{10.0}\right)$$
* **도약 거리 ($D_{spot}$, 셀 단위):**
  $$D_{spot} = 2 + \left\lfloor \text{random}() \times (W_{speed} \times 0.40) \right\rfloor$$
  * $25\text{m/s}$ 강풍 시 최대 12개 셀(약 $95\text{m}$) 전방으로 불씨가 도약.
* **착탄 좌표 계산:**
...
1. **연소 진행도 누적:**
   ...
   *(평균적으로 1개 셀은 약 14~18초 동안 지속적으로 타오릅니다.)*
2. **연소 온도 곡선 ($T_{flame} \in [0, 1]$):**
   $$T_{flame} = \sin\left(\min(1.0, \; \text{Progress})^{0.75} \times \pi\right) \times 0.85 + 0.15$$
   *(점화 초기에 빠르게 최고 온도에 도달하고 연소 완료 시 0으로 급랭)*
3. **완전 소진:**
   * $\text{Progress} \ge 1.0$ 도달 시 `CellStatus.BURNED`, 가연물 $\text{Fuel} = 0$, $T = 0$으로 전환되어 영구적으로 비활성화됩니다.

---

## 7. 거시적 관제 지표 (HUD Metrics) 산출 공식

| 지표 | 계산 공식 | 단위 |
| :--- | :--- | :--- |
| **소실 면적 (Burned Area)** | $\text{Burned Area} = N_{burned} \times A_{cell} \times \frac{1}{10,000}$ | $\text{ha}$ (헥타르) |
